...
//				LookupListItems item;
	...
	private static void parseValues(org.archiviststoolkit.structure.lookupListSchema.List list, LookupList lookupList, LookupListsDAO lookupListDAO) throws PersistenceException {
		...
		if (list.isAllowOtherOption() != null && list.isAllowOtherOption()) {
			lookupList.setIncludeOtherOption(true);
		} else {
			lookupList.setIncludeOtherOption(false);
		}
		//mark all of the items as editable and let the update remark the non
		//and set everything as not provisional
		for (LookupListItems thisItem : lookupList.getListItems()) {
			thisItem.setEditable(true);
			thisItem.setProvisional(false);
		}
	...
	public static Hashtable<String, TreeSet<LookupListItems>> getLookupListsObjects() {
		return lookupListsObjects;
	}

	public static boolean isAccumulateIngestReport() {
		return accumulateIngestReport;
	}
}
