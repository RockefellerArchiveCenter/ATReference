            ...
            public boolean verify(JComponent comp) {
                ...
                if (content.length() != 0) {
                    try {
                        ...
                        if (editedValue == null || editedValue < 0 || editedValue > ApplicationFrame.getInstance().getCurrentUserAccessClass())
                        {
                            JOptionPane.showMessageDialog(getParentEditor(), "Access Class must be between 0 and " + ApplicationFrame.getInstance().getCurrentUserAccessClass());
                            returnValue = false;
                        }
