...
//		dialogRepeatingData.setNewRecord(false);
	...
	protected void addDateActionPerformed(DomainSortableTable archDescriptionsDatesTable) {
		AccessionsResourcesCommon accessionsResourcesCommonModel = (AccessionsResourcesCommon) getModel();
		ArchDescriptionDates newArchDescriptionDates;
		DomainEditor dialogArchDescriptionDates = null;
		dialogArchDescriptionDates = new DomainEditor(ArchDescriptionDates.class, this.getParentEditor(), "Dates", new ArchDescriptionDatesFields());
		dialogArchDescriptionDates.setNavigationButtonListeners((ActionListener)this.getParentEditor());
		dialogArchDescriptionDates.setNewRecord(true);

		boolean done = false;
		int returnStatus;
		while (!done) {
			newArchDescriptionDates = new ArchDescriptionDates(accessionsResourcesCommonModel);
			dialogArchDescriptionDates.setModel(newArchDescriptionDates, null);
			returnStatus = dialogArchDescriptionDates.showDialog();
			if (returnStatus == JOptionPane.OK_OPTION) {
				accessionsResourcesCommonModel.addArchdescriptionDate(newArchDescriptionDates);
				archDescriptionsDatesTable.updateCollection(accessionsResourcesCommonModel.getArchDescriptionDates());
				done = true;
			} else if (returnStatus == StandardEditor.OK_AND_ANOTHER_OPTION) {
				accessionsResourcesCommonModel.addArchdescriptionDate(newArchDescriptionDates);
				archDescriptionsDatesTable.updateCollection(accessionsResourcesCommonModel.getArchDescriptionDates());
	...
	protected void removeDateActionPerformed(DomainSortableTable archDescriptionsDatesTable) {
		try {
			this.removeRelatedTableRow(archDescriptionsDatesTable, null, super.getModel());
		} catch (ObjectNotRemovedException e) {
			new ErrorDialog("Date not removed", e).showDialog();
		}
		archDescriptionsDatesTable.updateCollection(((AccessionsResourcesCommon) super.getModel()).getArchDescriptionDates());
	}
