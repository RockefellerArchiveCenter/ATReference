...
public class PatronManagement extends GeneralAdminDialog implements ActionListener {
    ...
    public static Color headerColor = null; // The color of the main header
...
//		dialog = DomainEditorFactory.getInstance().createDomainEditorWithParent(clazz, this, getContentTable(), true);
        ...
        // set the main header color
        headerColor = panel2.getBackground();
	}
    ...
    /**
     * Method to merge selected patron records
     */
    private void mergeButtonActionPerformed() {
        int[] selectedRows = patronTable.getSelectedRows();

		if (selectedRows.length < 2) {
			JOptionPane.showMessageDialog(this, "You must select at least 2 Patron records to merge");
		} else {
			Vector<DomainObject> selectedDomainObjects = new Vector<DomainObject>();

            for (int loop = 0; loop < selectedRows.length; loop++) {
				selectedDomainObjects.add(patronTable.getFilteredList().get(selectedRows[loop]));
			}

			SelectFromTable dialog = new SelectFromTable(clazz, this, "Select a Patron record to merge into", selectedDomainObjects);

            if (dialog.showDialog() == JOptionPane.OK_OPTION) {

				PatronsDAO dao  = new PatronsDAO();
    ...
    /**
     * Methods to merge the selected patron records in a seperate thread. It basically
     * the same code in the workspace dialog
     *
     * @param dialog
     * @param selectedDomainObjects
     * @param dao
     */
    private void mergeRecords(final SelectFromTable dialog,
							  final Vector<DomainObject> selectedDomainObjects,
							  final PatronsDAO dao) {

		final int numberOfRecords = selectedDomainObjects.size();

		final DomainObject selectedDomainObject = dialog.getSelectedValue();

        String message = "Are you sure you want to merge " + (numberOfRecords - 1) + " Patrons(s) into " + selectedDomainObject + "? \nThis will delete the other Patrons(s).";

		if (JOptionPane.showConfirmDialog(this, message, "Merge", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
			Thread performer = new Thread(new Runnable() {
				public void run() {
					InfiniteProgressPanel monitor = ATProgressUtil.createModalProgressMonitor(getSelf(), 1000);

                    monitor.start("Merging Patron Records...");

                    try {
						dao.merge(selectedDomainObjects, selectedDomainObject, monitor);

                        // go through and remove the patron records that where merged
                        for (DomainObject domainObject : selectedDomainObjects) {
							if (!domainObject.equals(selectedDomainObject)) {
								patronTable.getEventList().remove(domainObject);
							}
						}
    ...
    /**
     * A convenience method to return this dialog. This is mainly used within threads
     */
